...
<div class="flex grow flex-col overflow-y-auto bg-gradient-to-b from-primary-900 to-primary-800 px-6 pb-4 ring-1 ring-white/10 pt-0">
    <!-- Logo -->
    <a href="{{ route('dashboard') }}" class="flex shrink-0 items-center hover:opacity-80 transition-opacity py-3">
        ...
        <ul role="list" class="flex flex-1 flex-col gap-y-7">
            <li>
                <ul role="list" class="-mx-2 space-y-1">
                    ...
                    <!-- Planning / APP -->
                    @can('manage_annual_procurement_plans')
                    <li>
                        <a href="{{ route('annual-procurement-plans.index') }}"
                           class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('annual-procurement-plans.*') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                            <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                    ...
                    <li x-data="{ open: {{ request()->routeIs('requisitions.*', 'procurement.*', 'purchase-orders.*', 'grn.*', 'suppliers.*') ? 'true' : 'false' }} }">
                        <button @click="open = !open" 
                            ...
                            @if(isset($pendingRequisitions) && $pendingRequisitions > 0)
                                <span class="ml-auto mr-2 w-6 h-6 rounded-full bg-red-500 text-white text-xs flex items-center justify-center">{{ $pendingRequisitions }}</span>
                            @endif
                        ...
                        <ul x-show="open" x-cloak class="mt-1 px-2 space-y-1">
                            ...
                            <li>
                                <a href="{{ route('purchase-orders.index') }}" 
                                   class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('purchase-orders.*') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                                    Purchase Orders
                                </a>
                            </li>
                            @endcan
                            ...
                            <li>
                                <a href="{{ route('grn.index') }}" 
                                   class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('grn.*') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                                    Goods Received
                                </a>
                            </li>
                            @endcan
                            ...
                            <li>
                                <a href="{{ route('suppliers.index') }}"
                                   class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('suppliers.index', 'suppliers.show', 'suppliers.create', 'suppliers.edit') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                                    Suppliers
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('suppliers.asl.index') }}"
                                   class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('suppliers.asl.*') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                        ...
                        <ul x-show="open" x-cloak class="mt-1 px-2 space-y-1">
                            ...
                            <li>
                                <a href="{{ route('budgets.setup') }}" 
                                   class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('budgets.setup', 'budgets.department-setup') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                                    Budget Setup
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('budgets.dashboard') }}" 
                                   class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('budgets.dashboard') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                                    Budget Dashboard
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('budgets.index') }}" 
                                   class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('budgets.index', 'budgets.show', 'budgets.create', 'budgets.edit') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                                    Budget Lines
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('budgets.pending') }}" 
                                   class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('budgets.pending', 'budgets.show-approval', 'budgets.approve', 'budgets.reject') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                                    Pending Approvals
                                </a>
                            </li>
                            @endcan
                    ...
                    <li>
                        <a href="{{ route('capa.index') }}"
                           class="group flex gap-x-3 rounded-md p-2 text-sm leading-6 font-semibold {{ request()->routeIs('capa.*') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                            <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
                        ...
                        <ul x-show="open" x-cloak class="mt-1 px-2 space-y-1">
                            <li>
                                <a href="{{ route('reports.dashboard') }}"
                                   class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('reports.dashboard') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                                    KPI Dashboard
                                </a>
                            </li>
                            <li>
                                <a href="{{ route('reports.index') }}"
                                   class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('reports.index') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                                    All Reports
                                </a>
                            </li>
                        </ul>
                ...
                <ul x-show="open" x-cloak class="mt-1 px-2 space-y-1">
                    ...
                    <li>
                        <a href="{{ route('departments.index') }}" 
                           class="group flex gap-x-3 rounded-md p-2 pl-9 text-sm leading-6 font-semibold {{ request()->routeIs('departments.*') ? 'bg-primary-700 text-white' : 'text-primary-200 hover:text-white hover:bg-primary-700' }}">
                            <svg class="h-6 w-6 shrink-0" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor">
