...
<div class="max-w-7xl mx-auto">
    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            ...
            <p class="mt-2 text-sm text-gray-700">RFQ #{{ $process->process_number }}</p>
        </div>
        <div class="mt-4 sm:ml-16 sm:mt-0 flex gap-3">
            @if($process->status === 'draft')
                <a href="{{ route('procurement.rfq.edit', $process) }}" class="btn btn-secondary">Edit</a>
                <form action="{{ route('procurement.rfq.publish', $process) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="btn btn-primary">Publish RFQ</button>
                </form>
            @elseif(in_array($process->status, ['rfq_issued', 'bids_received']))
                <form action="{{ route('procurement.rfq.close', $process) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" class="btn btn-secondary">Close RFQ</button>
                </form>
            @endif
        </div>
    </div>
        ...
        <div class="lg:col-span-2 space-y-6">
            <div class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
                <div class="px-4 py-6 sm:p-8">
                    <h3 class="text-base font-semibold leading-7 text-gray-900">RFQ Details</h3>
                    <dl class="mt-6 space-y-6 divide-y divide-gray-100 text-sm leading-6">
                        ...
                        @if($requisition)
                        <div class="pt-6 sm:flex">
                            <dt class="font-medium text-gray-900 sm:w-64 sm:flex-none sm:pr-6">Linked Requisition</dt>
                            <dd class="mt-1 flex justify-between gap-x-6 sm:mt-0 sm:flex-auto">
                                <a href="{{ route('requisitions.show', $requisition) }}" class="text-primary-600 hover:text-primary-900">
            ...
            <div class="bg-white shadow-sm ring-1 ring-gray-900/5 sm:rounded-xl">
                <div class="px-4 py-6 sm:p-8">
                    <h3 class="text-base font-semibold leading-7 text-gray-900">Quotations Received ({{ $bids->count() }})</h3>
                    @if($bids->count() > 0)
                    ...
                    @else
                    <p class="mt-6 text-sm text-gray-500">No quotations received yet.</p>
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="mt-6">
        <a href="{{ route('procurement.indexRFQ') }}" class="text-sm font-semibold leading-6 text-gray-900">&larr; Back to RFQs</a>
    </div>
</div>
@endsection
