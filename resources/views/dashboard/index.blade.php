...
<div class="space-y-8">
    <!-- Page header -->
    <div>
        <h1 class="text-4xl font-bold text-gray-900">Welcome back, {{ auth()->user()->first_name }}!</h1>
            ...
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 flex items-baseline gap-2">
                        <span class="text-2xl font-bold text-gray-900">{{ $stats['pending_approvals'] ?? 12 }}</span>
        ...
        <div class="rounded-lg bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 flex items-baseline gap-2">
                        <span class="text-2xl font-bold text-gray-900">{{ $stats['active_requisitions'] ?? 28 }}</span>
        ...
        <div class="rounded-lg bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 flex items-baseline gap-2">
                        <span class="text-2xl font-bold text-gray-900">{{ $stats['budget_utilization'] ?? 72 }}%</span>
        ...
        <div class="rounded-lg bg-white p-6 shadow-sm hover:shadow-md transition-shadow">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="mt-2 flex items-baseline gap-2">
                        <span class="text-2xl font-bold text-gray-900">{{ $stats['low_stock_items'] ?? 7 }}</span>
                ...
                <div class="border border-gray-200 rounded-lg p-4 hover:border-primary-300 transition-colors">
                    <div class="flex items-start justify-between">
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-primary-600">{{ $requisition->requisition_number ?? 'N/A' }}</p>
                            <p class="mt-1 text-sm text-gray-600">{{ $requisition->purpose ?? 'No purpose' }}</p>
                            <p class="mt-2 text-xs text-gray-500">Department</p>
                        </div>
                        <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium ml-4 {{ $requisition->status_color ?? 'bg-gray-100 text-gray-700' }}">
                            {{ $requisition->status_label ?? 'Draft' }}
                        </span>
                    </div>
                </div>
                @empty
            ...
            <div class="space-y-4">
                ...
                <div class="border border-amber-200 bg-amber-50 rounded-lg p-4">
                    <div class="flex items-start gap-4">
                        ...
                        <div class="flex-1 min-w-0">
                            <p class="text-sm font-semibold text-gray-900">{{ $action['title'] ?? 'Approval Required' }}</p>
                            <p class="mt-1 text-sm text-gray-700">{{ $action['description'] ?? 'Requisition awaiting your approval' }}</p>
                            <a href="{{ $action['url'] ?? '#' }}" class="mt-3 inline-flex items-center text-sm font-semibold text-primary-600 hover:text-primary-700">
            ...
            <div class="space-y-5">
                ...
                <div>
                    <div class="flex justify-between items-center mb-2">
                        <span class="text-sm font-medium text-gray-700">{{ $budget['name'] ?? 'Department Budget' }}</span>
                        <span class="text-sm font-semibold text-gray-900">{{ $budget['percentage'] ?? 0 }}%</span>
                    </div>
                    <div class="w-full bg-gray-200 rounded-full h-2">
                        <div class="h-2 rounded-full {{ $budget['color'] ?? 'bg-primary-600' }}" style="width: {{ $budget['percentage'] ?? 0 }}%"></div>
                    </div>
                </div>
                @empty
                <div class="text-center py-8">
            ...
            <div class="space-y-4">
                ...
                <div class="flex gap-3">
                    <div class="flex-shrink-0">
                        <div class="flex h-8 w-8 items-center justify-center rounded-full bg-{{ $activity['color'] ?? 'gray' }}-100">
                            <svg class="h-4 w-4 text-{{ $activity['color'] ?? 'gray' }}-600" fill="currentColor" viewBox="0 0 20 20">
                                <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
                            </svg>
                        </div>
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-sm text-gray-700">{{ $activity['description'] ?? 'Activity description' }}</p>
                        <p class="mt-1 text-xs text-gray-500">{{ $activity['time'] ?? 'Just now' }}</p>
                    </div>
                </div>
                @empty
                <div class="text-center py-8">
