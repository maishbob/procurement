    ...
    <div class="sm:flex sm:items-center sm:justify-between">
        <div>
            <div class="flex items-center gap-x-3">
                <h1 class="text-3xl font-bold leading-tight text-gray-900">{{ $requisition->requisition_number }}</h1>
                <span class="inline-flex items-center rounded-full px-3 py-1 text-sm font-medium {{ $requisition->status_color }}">
            ...
            <p class="mt-2 text-sm text-gray-700">Created {{ $requisition->created_at->format('d M Y, H:i') }}</p>
        </div>
        <div class="mt-4 flex gap-x-3 sm:ml-16 sm:mt-0">
            ...
            @can('submit', $requisition)
                <form action="{{ route('requisitions.submit', $requisition) }}" method="POST" class="inline">
                    @csrf
                    <button type="submit" 
        ...
        <div x-show="activeTab === 'details'" class="space-y-6">
            <div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
                ...
                <div class="lg:col-span-2 space-y-6">
                        ...
                        <div class="px-6 py-5">
                            <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">{{ $requisition->department->name }}</dd>
                                </div>
                                <div>
                                ...
                                <div>
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900">{{ \Carbon\Carbon::parse($requisition->required_by_date)->format('d M Y') }}</dd>
                                </div>
                                <div>
                                ...
                                <div class="sm:col-span-2">
                                    ...
                                    <dd class="mt-1 text-sm text-gray-900 whitespace-pre-wrap">{{ $requisition->justification }}</dd>
                                </div>
                    ...
                    <!-- Special Flags -->
                    @if($requisition->is_emergency || $requisition->is_single_source)
                            ...
                            <div class="ml-3">
                                ...
                                <div class="mt-2 text-sm text-yellow-700 space-y-2">
                                    ...
                                    <div>
                                        ...
                                        <p>{{ $requisition->emergency_justification }}</p>
                                    </div>
                                    @endif
                                    @if($requisition->is_single_source)
                                    <div>
                                        <p class="font-medium">Single Source</p>
                                        <p>{{ $requisition->single_source_justification }}</p>
                                    </div>
                                    @endif
                                </div>
                            </div>
                    ...
                    <div class="rounded-lg bg-white shadow overflow-hidden">
                        ...
                        <div class="px-6 py-5 space-y-4">
                            <div class="flex justify-between">
                                ...
                                <span class="text-sm font-medium text-gray-900">{{ $requisition->formatted_estimated_total }}</span>
                            </div>
                            <div class="flex justify-between">
                                <span class="text-sm text-gray-500">VAT:</span>
                                <span class="text-sm font-medium text-gray-900">
                                    {{ number_format($requisition->items->sum(fn($item) => $item->is_vatable ? $item->estimated_total_price * 0.16 : 0), 2) }}
                                </span>
                            </div>
                            <div class="border-t border-gray-200 pt-4">
                                <div class="flex justify-between">
                                    ...
                                    <span class="text-base font-semibold text-primary-600">
                                        {{ $requisition->currency }} {{ number_format($requisition->estimated_total + $requisition->items->sum(fn($item) => $item->is_vatable ? $item->estimated_total_price * 0.16 : 0), 2) }}
                                    </span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="px-6 py-5">
                            <ul class="space-y-3">
                                <li class="flex items-center">
                                    <svg class="h-5 w-5 {{ $requisition->requires_hod_approval ? 'text-green-500' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
                                    </svg>
                                    <span class="ml-2 text-sm text-gray-700">HOD Approval</span>
                                </li>
                                <li class="flex items-center">
                                    <svg class="h-5 w-5 {{ $requisition->requires_principal_approval ? 'text-green-500' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
                                    </svg>
                                    <span class="ml-2 text-sm text-gray-700">Principal Approval</span>
                                </li>
                                <li class="flex items-center">
                                    <svg class="h-5 w-5 {{ $requisition->requires_board_approval ? 'text-green-500' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
                                    </svg>
                                    <span class="ml-2 text-sm text-gray-700">Board Approval</span>
                                </li>
                                <li class="flex items-center">
                                    <svg class="h-5 w-5 {{ $requisition->requires_tender ? 'text-green-500' : 'text-gray-300' }}" fill="currentColor" viewBox="0 0 20 20">
                                        <path fill-rule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.857-9.809a.75.75 0 00-1.214-.882l-3.483 4.79-1.88-1.88a.75.75 0 10-1.06 1.061l2.5 2.5a.75.75 0 001.137-.089l4-5.5z" clip-rule="evenodd" />
                                    </svg>
                ...
                <table class="min-w-full divide-y divide-gray-300">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4 text-sm text-gray-500 max-w-xs truncate">{{ $item->specifications }}</td>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900 text-right">{{ number_format($item->estimated_unit_price, 2) }}</td>
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900 text-right">{{ number_format($item->estimated_total_price, 2) }}</td>
                    ...
                    <tfoot class="bg-gray-50">
                        <tr>
                            ...
                            <td class="px-6 py-4 whitespace-nowrap text-sm font-bold text-gray-900 text-right">{{ $requisition->formatted_estimated_total }}</td>
                            <td></td>
                        </tr>
                    </tfoot>
                </table>
        ...
        <div x-show="activeTab === 'approvals'" class="space-y-6">
            ...
            @can('approve', $requisition)
                <div class="rounded-lg bg-primary-50 border border-primary-200 p-6">
                    ...
                    <form action="{{ route('requisitions.approve', $requisition) }}" method="POST" class="space-y-4">
                        @csrf
                        <div>
                    ...
                    <form id="reject-form" action="{{ route('requisitions.reject', $requisition) }}" method="POST" class="mt-4 hidden border-t border-primary-200 pt-4">
                        @csrf
                        <div class="space-y-4">
                            <div>
                                ...
                                <li>
                                    <div class="relative pb-8">
                                        ...
                                        <div class="relative flex items-start space-x-3">
                                            <div class="relative">
                                                <div class="flex h-10 w-10 items-center justify-center rounded-full {{ $approval->status === 'approved' ? 'bg-green-500' : ($approval->status === 'rejected' ? 'bg-red-500' : 'bg-gray-400') }} ring-8 ring-white">
                                            ...
                                            <div class="min-w-0 flex-1">
                                                <div>
                                                    <div class="text-sm">
                                                        <span class="font-medium text-gray-900">{{ $approval->level_label }}</span>
                                                    </div>
                                                    <p class="mt-0.5 text-sm text-gray-500">
                                                        {{ $approval->approver->full_name }}
                                                        <span class="inline-flex items-center rounded-full px-2 py-0.5 text-xs font-medium {{ $approval->status_color }} ml-2">
                                ...
                                <div class="relative flex items-start space-x-3">
                                    ...
                                    <div class="min-w-0 flex-1 py-1.5">
                                        <div class="text-sm text-gray-500">
                                            <span class="font-medium text-gray-900">{{ $requisition->requester->full_name }}</span>
                                            {!! ' created the requisition' !!}
                                            <span class="whitespace-nowrap">{{ $requisition->created_at->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="relative flex items-start space-x-3">
                                    ...
                                    <div class="min-w-0 flex-1 py-1.5">
                                        <div class="text-sm text-gray-500">
                                            ...
                                            <span class="whitespace-nowrap">{{ \Carbon\Carbon::parse($requisition->submitted_at)->diffForHumans() }}</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
